...
void Add_Slic3rExPolygon_to_ClipperPaths(const Slic3r::ExPolygon &input, ClipperLib::Paths* output) {
    output->push_back(Slic3rMultiPoint_to_ClipperPath(input.contour));
    for (Slic3r::Polygons::const_iterator it = input.holes.begin(); it != input.holes.end(); ++it) {
        output->push_back(Slic3rMultiPoint_to_ClipperPath(*it));
    }
}
...
template <class T>
T
_clipper_do(ClipperLib::Clipper &clipper, const ClipperLib::ClipType clipType, const Polygons &subject, 
    ...
    const Polygons &clip, bool safety_offset_)
{
    // perform operation
    ClipperLib::Clipper clipper(global_manager);
    ClipperLib::Paths output = _clipper_do<ClipperLib::Paths>(clipper, clipType, subject, clip, ClipperLib::pftNonZero, safety_offset_);
    clipper.Clear();
    ...
    const Polygons &clip, bool safety_offset_)
{
    // perform operation
    ClipperLib::Clipper clipper(global_manager);
    ClipperLib::PolyTree *polytree = _clipper_do_to_polytree(clipper, clipType, subject, clip, ClipperLib::pftNonZero, safety_offset_);
...
ClipperLib::PolyTree*
union_pt(ClipperLib::Clipper &clipper, const Polygons &subject, bool safety_offset_)
{
    return _clipper_do<ClipperLib::PolyTree*>(clipper, ClipperLib::ctUnion, subject, Polygons(), ClipperLib::pftEvenOdd, safety_offset_);
}
...
{
    ClipperLib::Clipper clipper(global_manager);
    ClipperLib::PolyTree *polytree = union_pt(clipper, subject, safety_offset_);

...
//        );
    ...
    for (Surfaces::const_iterator surface_it = surfaces.surfaces.begin();
        ...
        if (this->layer()->object()->no_model_object() && (f->spacing() < 0)) {
          fprintf(stderr, "Slic3r was called with --import-paths and has computed negative spacing for a filling. This will likely hang the computer while allocating huge amounts of memory, so the slicing is being aborted right now. This is very probably caused by incorrect Z spacing in the imported paths.");
          fflush(stderr);
          std::terminate();
        }        

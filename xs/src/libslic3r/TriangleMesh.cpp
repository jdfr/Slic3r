...
template <Axis A>
void
TriangleMeshSlicer<A>::slice(std::vector<float> &z, std::vector<Polygons>* layers) const
{
    /**
    ...
    */
    ...
    std::vector<int> layers_indexes;
    if (!IsSorted(z)) {
      std::vector<SortPair> pairs;
      pairs.resize(z.size());
      layers_indexes.resize(z.size());
      ...
      for (int k=0; k<z.size(); ++k) {
        z[k] = pairs[k].first;
        layers_indexes[k] = pairs[k].second;
      }
    }
    
    {
        ...
        parallelize<int>(
            ...
            boost::bind(&TriangleMeshSlicer<A>::_slice_do, this, _1, &lines, &lines_mutex, z, layers_indexes)
        );
    }
...
TriangleMeshSlicer<A>::_slice_do(size_t facet_idx, std::vector<IntersectionLines>* lines, boost::mutex* lines_mutex, 
    const std::vector<float> &z, const std::vector<int> &layers_indexes) const
{
    ...
    bool use_layer_idxs = !layers_indexes.empty();
    for (std::vector<float>::const_iterator it = min_layer; it != max_layer + 1; ++it) {
        std::vector<float>::size_type layer_idx = it - z.begin();
        if (use_layer_idxs) { layer_idx = layers_indexes[layer_idx]; }
...
template <Axis A>
void
TriangleMeshSlicer<A>::slice(std::vector<float> &z, std::vector<ExPolygons>* layers) const
{
...
TriangleMeshSlicer<A>::make_expolygons(const Polygons &loops, ExPolygons* slices) const
{
    ...
    */
    ...
    double safety_offset = this->safety_offset;
...
template <Axis A>
TriangleMeshSlicer<A>::TriangleMeshSlicer(TriangleMesh* _mesh, double _safety_offset) : mesh(_mesh), safety_offset(_safety_offset), v_scaled_shared(NULL)
{
